from ultrasonic.hcsr04 import HCSR04
import machine
import time
import _thread
...
class RobotUltrasonic:
    def __init__(self):
        ...
        self.sensor_middle = HCSR04(trigger_pin=17, echo_pin=5, echo_timeout_us=30000)
        self.sensor_left = HCSR04(trigger_pin=15, echo_pin=2, echo_timeout_us=30000)
        self.sensor_right = HCSR04(trigger_pin=4, echo_pin=16, echo_timeout_us=30000)
    ...
    def distance(self, sensor, num_samples=10):
        total_distance = 0
    ...
    def adc(self, reading):
        threshold = 16
        return 1 if reading > threshold else 0
    
    def ultra_data(self):
        middle = self.adc(self.distance(self.sensor_middle, 2))
        left = self.adc(self.distance(self.sensor_left, 2))
        right = self.adc(self.distance(self.sensor_right, 2))
